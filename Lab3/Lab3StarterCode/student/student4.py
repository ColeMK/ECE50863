...
THROUGHPUT_IN_SIZE = 15
THROUGHPUT_OUT_SIZE = 5
TRAIN_PER_STEP = 3
PLOT = False
LOAD_MODEL = True
SAVE_MODEL = True
MODEL_PTH = os.path.join('models', os.environ['model_name']) + '.pth'
...
# Parameters
input_size = 2  # Size of each input item
...
def student_entrypoint(client_message: ClientMessage):
	...
	if len(prediction_buffer) > 1:
		for i in range(TRAIN_PER_STEP):
			...
			input = torch.stack((torch.tensor(inputThroughputs), torch.tensor(buffers)), dim=1)
