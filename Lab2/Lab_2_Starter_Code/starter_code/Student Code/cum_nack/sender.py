...
class send_packets:
		...
		while True:
			...
			if raw_data != dataBuffer:
				if data[0] == "END": 
					...
					self.cum_index = int(data[2])
					self.timer_thread.cancel()
					self.multiple = int(data[1])
					self.new_send_func(data[4])
				# if data[0] == "COM":
				# 	self.timer_thread.cancel()
				# 	self.send_func()
			dataBuffer = raw_data
	...
	def new_send_func(self,data):
		...
		for i in data:
			print(i)
			packet_num = self.last_recieved - (self.cum_index - 1)+ i#self.window_size * self.multiple + i
			if packet_num < totpackets:
				resend_packet = self.packet_list[packet_num]
				print(f"{self.curr_packet_id}:{self.packets_left}:{window_size}")
				msg = (f"{packet_num}|{window_size}|").encode('utf-8') + resend_packet
				print(f"resend packet num {packet_num}")
				self.send_monitor.send(self.reciever_id,msg)
		...
		self.prev_window_size = window_size
		#send data from old threshold to new
