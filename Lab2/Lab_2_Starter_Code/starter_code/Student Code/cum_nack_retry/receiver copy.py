...
class packet_reciever:
	...
	def send_nack(self): #might send twice for redundancy
		...
		self.buff_window = self.window[0:self.nack_index]
		...
		# self.timer_init()
		self.nack_timer_live = 0
	def packet_handler_init(self,data):
		self.send_thread = threading.Thread(target=self.packet_handler, args=[data])
		self.send_thread.start()
	def packet_handler(self, data):
