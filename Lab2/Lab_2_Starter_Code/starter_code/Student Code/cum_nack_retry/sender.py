...
class send_packets:
	def __init__(self, packet_list, timeout, send_monitor, receiver_id, window_size, max_packet_size):
		...
		self.tot_packets = len(packet_list)
		...
		while True:
			...
			if raw_data != dataBuffer:
				if data[0] == "E": 
					...
					#print(f"Ending: {time.time()}")
					send_monitor.send_end(receiver_id)
					#print(self.overheard)
					break
				else:
					self.last_recieved = int(data[1])
					self.timer_thread.cancel()
					if self.last_recieved != -1:
						self.new_send_func(data[2])
					else:
						#send what was previously sent already
						if self.prev_data != 0:
							self.packets_left = self.prev_packets_left
							self.sent_num = self.prev_sent_num
							self.new_send_func(self.prev_data)
						else:
							self.send_func()
			dataBuffer = raw_data

	def new_send_func(self,data):
		#resend nacks
		if type(data) != list:
			data = ast.literal_eval(data)
		else:
		...
		#data = [eval(i) for i in data]
		#print(f"In data: {data}, Multiple {self.multiple}")
		#window_range = self.last_recieved + 1
		if len(data)>=1:
			window_size = data[0] + len(data)
			window_range = min(self.window_size, data[0] + self.packets_left)
		else:
			window_size = self.window_size
			window_range = min(self.window_size, self.packets_left)

		
		#window_size = min(self.window_size, self.packets_left+len(data))
		# if self.packets_left < window_size:
		# 	window_size = self.packets_left + len(data)
		self.prev_data = data
		self.prev_packets_left = self.packets_left
		self.prev_sent_num = self.sent_num

		for i in data:
		#	print(i)
			packet_num = self.last_recieved + (i - data[0] + 1) #self.window_size * self.multiple + i
			if packet_num < totpackets:
				resend_packet = self.packet_list[packet_num]
			#	print(f"{packet_num}:{self.packets_left}:{window_size}")
				msg = (f"{packet_num}|{window_range}|").encode('utf-8') + resend_packet
				#self.overheard += len(msg)
			#	print(f"resend packet num {packet_num}")
				self.send_monitor.send(self.reciever_id,msg)
		#iterate cum_ack to + packet num recieved
		sent = 0
		for i in range(min(window_size-len(data), self.packets_left)):
			if self.packets_left != 0:
				self.current_packet = self.packet_list[self.sent_num]
				self.curr_packet_id = self.sent_num
			#	print(f"{self.curr_packet_id}:{self.packets_left}:{window_size}")
				msg = (f"{self.curr_packet_id}|{window_range}|").encode('utf-8') + self.current_packet
				#self.overheard += len(msg)
				#print(len(msg))
				self.send_monitor.send(self.reciever_id,msg)
				self.sent_num += 1
	...
	def send_func(self): 
		self.prev_data = 0
		self.sent_num = 0
		self.packets_left = self.tot_packets
		window_size = self.window_size
		if self.packets_left < window_size:
	...
	def start_up(self): #starts the trheads
		...
		#self.send_thread = SendThread(target=self.sFend_func)
