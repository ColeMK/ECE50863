...
class packet_reciever:
	...
	def write_to_file(self, buff_window, write_location):
		...
		for i,p in enumerate(buff_window):
			...
			if p!= -1 and self.tracking_pack_buff[i] == self.next_write:
	...
	def start_up(self):
		...
		while self.end != 1: # may be -1
			...
			if self.packet_id != 'END':
				...
				# while (self.window[self.packet_id%self.window_size-self.cum_index] != -1 and self.tracking_pack[self.packet_id%self.window_size-self.cum_index] != self.packet_id):
				# 	print(self.packet_id)
				# 	time.sleep(.1)
				# 	_ = 1
				...
				# if self.packet_id+1 % 20:
				# 	if self.nack_timer_live == 1:
